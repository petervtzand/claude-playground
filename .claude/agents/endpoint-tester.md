...
4. **Run `task test`** to verify the new tests pass. If any test fails, read the failure output, fix the assertion (e.g. wrong expected status code or body), and re-run. Iterate until green.
...
- **Only** run `task test` (with optional pytest args). Do not run other Bash commands — they will prompt Peter and get denied.
